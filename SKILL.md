...
description: "Use when tasks require fetching market data from FRED, VIXCentral, Macrotrends, Yahoo Finance, Westmetall, EIA DNAV pages, or Multpl. Prefer direct FRED CSV URLs, Macrotrends `/economic-data/{page_id}/{chart_frequency}` JSON, Westmetall chart XML from `/api/marketdata/{lang}/{field}/`, EIA linked XLS/history pages, Multpl inline `pi` chart payloads and table views, VIXCentral term-structure and historical routes with browser fallback when needed, and the `dachent/yf_marketdata` CLI for Yahoo Finance exports."
---

# Fetch Cross-Market Data

Use this skill when a task needs raw downloads, parsed tables, or provider-aware analysis from FRED, VIXCentral, Macrotrends, Yahoo Finance, Westmetall, EIA DNAV pages, or Multpl.
...
- VIXCentral
  ...
  The script tries direct HTTP first and falls back to Playwright-driven browser fetches when VIXCentral returns placeholders or non-JSON.
- Westmetall
...
- Open `references/vixcentral.md` for route behavior, array shapes, and browser-fallback notes.
...
- If VIXCentral direct calls return `"hello"` or `"hello historical"`, let the helper script switch to Playwright instead of inventing data.
