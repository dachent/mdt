...
The skill is native-first. It prefers direct machine-readable provider sources such as FRED CSV, Macrotrends JSON, Westmetall chart XML, linked EIA XLS workbooks for history pages, VIXCentral XHR-style routes, and Multpl's inline chart payload.
...
- EIA history parsing currently prefers linked `.xls` workbooks and relies on Windows plus Excel COM for structured history extraction.
